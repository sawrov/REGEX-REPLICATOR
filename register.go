...
value := flag.String("v","","Enter the  value/name of the registry entry")
...
if *add_reg!=""{
	full_path=*add_reg
	if key,ok:=key_dict[strings.ToUpper(strings.Split(full_path,"\\")[0])];ok{
		err=add_reg_value(key,full_path,*value,*data_type,*data,)
	}else{
		fmt.Println("Please enter correct registry path")
		os.Exit(1)
	}
	...
	full_path=*del_reg
	if key,ok:=key_dict[strings.ToUpper(strings.Split(full_path,"\\")[0])];ok{
		err=delete_reg_key(key,full_path)
	}else{
		fmt.Println("Please enter correct registry path")
		os.Exit(1)
	}
	...
	full_path=*query_reg
	if key,ok:=key_dict[strings.ToUpper(strings.Split(full_path,"\\")[0])];ok{
		err=query_reg_value(key,full_path,*value,*recursive_check)
	}else{
		fmt.Println("Please enter correct registry path")
		os.Exit(1)
	}
